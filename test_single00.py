...
for (word, index) in words:
    ...
    if y < 0.5:
        pred_confidence = 1 - y
        prediction_is_correct = index in [0,1] or word in dicts[0] or word in dicts[1]
    else:
        pred_confidence = y
        prediction_is_correct = index in [2,3] or word in dicts[2] or word in dicts[3]
...
plt.legend(loc = 'best')
plt.savefig('cov_acc_1_single.png')
...
plt.legend(loc = 'best')
plt.savefig('cov_acc_2_single.png')